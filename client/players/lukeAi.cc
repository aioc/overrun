...
#define SOLDIER_LEVEL            51
#define BASE_KILLER              200
#define WORKER_PER_PATCH         0.1
#define SOLDIER_PER_UNIT         1.0
#define HARRASER_PER_WORKER      0.65
#define LARGE_LEVEL              200000000
...
int isSomeoneClose (int sX, int sY);
...
void clientDoTurn() {
   ...
   int soldierDif = (((int) (((double) (amoEnSold)) * SOLDIER_PER_UNIT)) - amoSoldiers);
   int harrasDif = (((int) (((double) (amoEnWork)) * HARRASER_PER_WORKER)) - amoHarras);

   printf ("%d %d %d\n", workerDif, soldierDif, harrasDif);
   if (workerDif > soldierDif && workerDif > harrasDif && (playersMinerals[myID] <= 800 || amoWorkers < 5)) {
      if (playersMinerals[myID] < WORKER_LEVEL) {
         build (playersMinerals[myID]);
      } else {
         build (getCost (WORKER_LEVEL));
      }
   } else if (soldierDif > harrasDif || playersMinerals[myID] < getCost (HARRAS_LEVEL) || amoSoldiers < 5) {
      build (getCost (SOLDIER_LEVEL));
   } else {
...
int moveWorker (int num) {
   ...
   int cl = isSomeoneClose (allUnits[num].x, allUnits[num].y);
   if (cl != -1) {
      return cl;
...
int moveSoldier (int num) {
   // Time to attack!
   struct point p = findClosestBelow (allUnits[num].x, allUnits[num].y, allUnits[num].level);
   if (p.x != -1) {
      return findDir (allUnits[num].x, allUnits[num].y, p.x, p.y, allUnits[num].level);
   } else {
      // Suicide!
      p = findClosestBelow (allUnits[num].x, allUnits[num].y, LARGE_LEVEL);
      if (p.x == -1) {
         // Make random move
         return rand () % 4;
      } else {
         return findDir (allUnits[num].x, allUnits[num].y, p.x, p.y, LARGE_LEVEL);
      }
   }
}
...
int isSomeoneClose (int sX, int sY) {
   ...
   for (i = 0; i < 4; i++) {
      ...
      newY = sY + dy[i];
      if (newX >= 0 && newY >= 0 && newX < size && newY < size) {
         if (whatOnSquare[newY][newX].ownerID != myID && whatOnSquare[newY][newX].level > whatOnSquare[sY][sX].level) {
            for (j = 0; j < 4; j++) {
               int nx = sX + dx[j];
               int ny = sY + dy[j];
               if (nx >= 0 && ny >= 0 && nx < size && ny < size) {
                  if (whatOnSquare[ny][nx].level <= 0 && terrain[ny][nx] != WALL) {
                     return j;
                  }
               }
            }
...
void clientDroneLocation(int pid, int id, int x, int y, int level) {
   ...
   if (pid == myID) {
      ...
      if (amoStill <= 0 || stillUnits[amoStill - 1].unitID != id) {
         amoEnSold++;
      }
      amoOthers++;
   }
